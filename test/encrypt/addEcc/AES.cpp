...
byte AES::set_key (byte ret [], int keylen)
{
  byte hi ;
  switch (keylen)
    {
  ...
  hi = (round + 1) << 4 ;
  copy_n_bytes (key_sched, ret, keylen) ;
  byte t[4] ;
  byte next = keylen ;
...
void AES::do_aes_encrypt(byte *plain,int size_p,byte *cipher,byte *ret, int bits, byte ivl [N_BLOCK]){
...
void AES::do_aes_encrypt(byte *plain,int size_p,byte *cipher,byte *ret, int bits){
  calc_size_n_pad(size_p);
  byte plain_p[get_size()];
  padPlaintext(plain,plain_p);
  int blocks = get_size() / N_BLOCK;
...
void AES::do_aes_decrypt(byte *cipher,int size_c,byte *plain,byte *ret, int bits, byte ivl [N_BLOCK]){
  set_size(size_c);
...
void AES::do_aes_decrypt(byte *cipher,int size_c,byte *plain,byte *ret, int bits){
  set_size(size_c);
  int blocks = size_c / N_BLOCK;
  set_key (ret, bits);
